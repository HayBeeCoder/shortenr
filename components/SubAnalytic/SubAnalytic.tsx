...
const SubAnalytic = ({toolTipMessage, title, children ,special,should_flex}: IProps) => {
    ...
    return (
        ...
        <div className={`py-7  h-full bg-[#FFF] rounded-md  ${special ? '' : 'px-20 max-w-[350px]  mx-auto md:max-w-none aspect-square md:aspect-auto md:h-full ' } ${should_flex ? "flex flex-col" : ""}`}>
            ...
            <div className={`w-full ${special ? "overflow-x-scroll md:overflow-x-auto" : ''}`}>
                ...
                <div className={`${special ? "w-[1000px] md:w-full" : ""}`}>
